...
[CustomEditor(typeof(SelectTargetCore), true)]
public class SelectTargetCoreEditor : Editor
{
    ...
    private SelectTargetCore mTarget;
    ...
    private void OnEnable()
    {
        ...
        mTarget = (SelectTargetCore)target;
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorHelper.DrawInspectorTitle("ootii Select Target Core");
        ...
        if (EditorHelper.BoolField("Continuous Select", "Determines if we continually select a target based on the ray or only select when the Action Alias is pressed.", mTarget.ContinuousSelect, mTarget))
        {
            mIsDirty = true;
            mTarget.ContinuousSelect = EditorHelper.FieldBoolValue;
        }
        ...
        EditorGUILayout.LabelField("Filters", EditorStyles.boldLabel, GUILayout.Height(16f));
        ...
        if (EditorHelper.BoolField("Actor Core Required", "Determines if an Actor Core is required to be a target.", mTarget.RequiresActorCore, mTarget))
        {
            mIsDirty = true;
            mTarget.RequiresActorCore = EditorHelper.FieldBoolValue;
        }

        if (EditorHelper.BoolField("Combatant Required", "Determines if a Combatant is required to be a target.", mTarget.RequiresCombatant, mTarget))
        {
            mIsDirty = true;
            mTarget.RequiresCombatant = EditorHelper.FieldBoolValue;
        }
        ...
        if (EditorHelper.ObjectField<Material>("Highlight Material", "Material used to highlight the target.", mTarget.HighlightMaterial, mTarget))
        {
            mIsDirty = true;
            mTarget.HighlightMaterial = EditorHelper.FieldObjectValue as Material;
        }

        Color lNewColor = EditorGUILayout.ColorField(new GUIContent("Highlight Color", "Color to apply to the material (if used)."), mTarget.HighlightColor);
        if (mTarget.HighlightColor != lNewColor)
        {
            mIsDirty = true;
